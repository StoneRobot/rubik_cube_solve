...
void RubikCubeSolve::photographPickPlace(int robotNum, geometry_msgs::PoseStamped& pose, bool isPick, int pre_grasp_approach[], int post_grasp_retreat[])
{
    ...
    { 
        ...
        // 寸进的过程 前进1
        robotMoveCartesian(getMoveGroup(robotNum), pre_grasp_approach[0]*prepare_some_distance, \
        ...
        // 寸进的过程 后退
        robotMoveCartesian(getMoveGroup(robotNum), post_grasp_retreat[0]*prepare_some_distance, \
...
bool RubikCubeSolve::recordPose(int robotNum, std::string name, bool isJointSpce=false)
{
    ...
    if(isJointSpce)
    {
        ROS_ERROR_STREAM("record joint space in development ...");
    }
    else
    {
        geometry_msgs::PoseStamped pose;
...
int RubikCubeSolve::Cartesian()
{
    ...
    {
        robotMoveCartesian(move_group0, 0, 0, -prepare_some_distance);
    }
    return 0;
}
                            ...
                            bool only_plan=true)
{
    ...
    // {
    //     ROS_ERROR_STREAM(i);
    //     ROS_INFO_STREAM(multi_plan.trajectory_.joint_trajectory.points[i]);
    // }
